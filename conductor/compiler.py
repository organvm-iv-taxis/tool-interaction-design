...
class WorkflowCompiler:
    ...
    def compile_mission(
        ...
        self._simulate_and_harden(state, cluster_path, idx_to_name)
        
        # 4. Inject into executor (persist it)
        self.executor.save_state(state)
        return state

    def _simulate_and_harden(self, state: WorkflowState, cluster_path: list[str], idx_to_name: dict[int, str] | None = None) -> None:
        ...
        if avg_health < 0.85:
            ...
            # Use idx_to_name mapping if available, otherwise fall back to step_order
            if idx_to_name and weakest_idx in idx_to_name:
                target_step_name = idx_to_name[weakest_idx]
            elif weakest_idx < len(state.step_order):
                target_step_name = state.step_order[weakest_idx]
            else:
