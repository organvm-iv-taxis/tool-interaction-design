...
"""
        ...
        # 6. CODEOWNERS files from seed.yaml ownership
        codeowners = self.generate_codeowners()
        for organ_key, content in codeowners.items():
            short = organ_short(organ_key)
            artifacts.append((f"codeowners/{short}-CODEOWNERS", content))

        # 7. GitHub Repository Rulesets (M9)
        if not dry_run:
            try:
                gh_rulesets = self.generate_github_rulesets()
                for org_name, payload in gh_rulesets.items():
                    artifacts.append((f"github-rulesets/{org_name}.json", payload))
            except GovernanceError:
                pass  # No governance data — skip rulesets

            # 8. Tier-based CI templates (M14)
            ci_templates = self.generate_ci_templates()
            for tier, content in ci_templates.items():
                artifacts.append((f"ci-templates/{tier}.yml", content))
        else:
            # In dry_run, report what would be generated without writing
            artifacts.append(("github-rulesets/*.json", "(GitHub rulesets per org)"))
            for tier in ["flagship", "standard", "stub", "infrastructure"]:
                artifacts.append((f"ci-templates/{tier}.yml", "(CI workflow template)"))
    ...
    def compliance_rate(self) -> dict[str, Any]:
        """Compute per-organ compliance rates for seed.yaml, CI workflow, and README presence."""
        organ_rates: dict[str, dict[str, Any]] = {}
        total_repos = 0
        total_seed = 0
        total_ci = 0
        total_readme = 0
        ...
        for organ_key, organ_data in self.registry.get("organs", {}).items():
            ...
            count = len(repos)
            if count == 0:
                organ_rates[organ_key] = {
                    "total": 0, "seed_pct": 0.0, "ci_pct": 0.0, "readme_pct": 0.0,
                }
                continue
            ...
            for repo in repos:
                repo_name = repo.get("name", "")
                repo_path = self._find_repo_path(repo_name, organ_key)

                if repo_path and repo_path.is_dir():
                    if (repo_path / "seed.yaml").is_file():
                        seed_count += 1
                    ...
                    if bool(str(repo.get("ci_workflow", "")).strip()):
                        ci_count += 1
                    if str(repo.get("documentation_status", "")).upper() != "EMPTY":
                        readme_count += 1

            total_repos += count
            total_seed += seed_count
            total_ci += ci_count
            total_readme += readme_count

            organ_rates[organ_key] = {
                "total": count,
                "seed_pct": round(seed_count / count * 100, 1),
                "ci_pct": round(ci_count / count * 100, 1),
                "readme_pct": round(readme_count / count * 100, 1),
            }

        overall = {
            "total_repos": total_repos,
            "seed_pct": round(total_seed / total_repos * 100, 1) if total_repos else 0.0,
            "ci_pct": round(total_ci / total_repos * 100, 1) if total_repos else 0.0,
            "readme_pct": round(total_readme / total_repos * 100, 1) if total_repos else 0.0,
        }

        return {"organs": organ_rates, "overall": overall}

    def generate_codeowners(self) -> dict[str, str]:
        """Generate CODEOWNERS content from seed.yaml ownership data.

        Returns a dict mapping organ_key to CODEOWNERS file content.
        """
        result: dict[str, str] = {}

        for organ_key, organ_data in self.registry.get("organs", {}).items():
            lines = ["# CODEOWNERS - auto-generated by conductor governance", ""]
            repos = organ_data.get("repositories", [])
            ...
            for repo in repos:
                ...
                if repo_path and repo_path.is_dir():
                    ...
                    if seed_file.is_file():
                        try:
                            seed = yaml.safe_load(seed_file.read_text()) or {}
                            owner = seed.get("owner") or seed.get("maintainer")
                        except Exception:
                            pass
    ...
    # ----- M9: GitHub Repository Rulesets -----

    def generate_github_rulesets(self) -> dict[str, Any]:
        """Generate org-level GitHub Repository Ruleset JSON from governance-rules.json.

        Returns a dict mapping org name to ruleset payload. Also writes JSON
        files to generated/github-rulesets/.
        """
        if not self.governance and not self.registry:
            raise GovernanceError("No governance rules or registry loaded.")

        rulesets: dict[str, Any] = {}

        # Identify flagship and PUBLIC_PROCESS+ repos per organ
        flagship_repos: dict[str, list[str]] = {}
        public_process_plus: dict[str, list[str]] = {}
        ...
        for organ_key, organ_data in self.registry.get("organs", {}).items():
            ...
            for repo in organ_data.get("repositories", []):
                name = repo.get("name", "")
                tier = str(repo.get("tier", "")).lower()
                status = str(repo.get("promotion_status", "")).upper()
                if tier == "flagship":
                    flagships.append(name)
                if status in {"PUBLIC_PROCESS", "GRADUATED"}:
                    pp_repos.append(name)
            flagship_repos[org_name] = flagships
            public_process_plus[org_name] = pp_repos

        for short_key, meta in ORGANS.items():
            org = meta["org"]
            rules: list[dict[str, Any]] = []

            # Rule 1: Block force push on default branches (all repos)
            rules.append({
                "type": "non_fast_forward",
            })

            # Rule 2: Require PRs for flagship repos
            if flagship_repos.get(org):
                rules.append({
                    "type": "pull_request",
                    "parameters": {
                        "required_approving_review_count": 0,
            ...
            # Rule 3: Require signed commits for PUBLIC_PROCESS+ repos
            if public_process_plus.get(org):
                rules.append({
                    "type": "required_signatures",
                })

            ruleset = {
                "name": f"{org}-governance-ruleset",
                "target": "branch",
                "enforcement": "active",
                "conditions": {
                    "ref_name": {"include": ["~DEFAULT_BRANCH"], "exclude": []},
                ...
                "metadata": {
                    ...
                    "flagship_repos": flagship_repos.get(org, []),
                    "public_process_repos": public_process_plus.get(org, []),
                },
            }
            rulesets[org] = ruleset

        # Write to generated/github-rulesets/
        out_dir = GENERATED_DIR / "github-rulesets"
        out_dir.mkdir(parents=True, exist_ok=True)
        for org_name, payload in rulesets.items():
            out_path = out_dir / f"{org_name}.json"
            atomic_write(out_path, json.dumps(payload, indent=2))

        log_event("governance.github_rulesets_generated", {"count": len(rulesets)})
        return rulesets

    # ----- M14: Tier-based CI template generation -----

    def generate_ci_templates(self) -> dict[str, str]:
        """Generate differentiated CI workflow YAML files per tier.

        Returns a dict mapping tier name to YAML content. Also writes files
        to generated/ci-templates/.
        """
        templates: dict[str, dict[str, Any]] = {}

        # Flagship: full test matrix
        templates["flagship"] = {
            "name": "CI — Flagship",
            ...
            "jobs": {
                ...
                "typecheck": {
                    ...
                    "steps": [
                        {"uses": "actions/checkout@v4"},
                        {"name": "Type check", "run": "echo 'Run type checker'"},
                    ],
                },
                "unit-tests": {
                    "runs-on": "ubuntu-latest",
                    ...
                    "steps": [
                        {"uses": "actions/checkout@v4"},
                        {"name": "Unit tests", "run": "echo 'Run unit tests'"},
                    ],
                },
                "integration-tests": {
                    "runs-on": "ubuntu-latest",
                    ...
                    "steps": [
                        {"uses": "actions/checkout@v4"},
                        {"name": "Integration tests", "run": "echo 'Run integration tests'"},
                    ],
                },
                "coverage": {
                    "runs-on": "ubuntu-latest",
                    ...
                    "steps": [
                        {"uses": "actions/checkout@v4"},
                        {"name": "Coverage", "run": "echo 'Run coverage report'"},
                    ],
                },
            },
        }

        # Standard: lint + unit tests
        templates["standard"] = {
            "name": "CI — Standard",
            "on": {"push": {"branches": ["main"]}, "pull_request": {"branches": ["main"]}},
            "jobs": {
        ...
        templates["infrastructure"] = {
            ...
            "jobs": {
                "validate": {
                    ...
                    "steps": [
                        {"uses": "actions/checkout@v4"},
                        {"name": "Validate schemas", "run": "echo 'Run schema/config validation'"},
                    ],
                },
            },
        }
        ...
        # Write YAML files
        out_dir = GENERATED_DIR / "ci-templates"
        out_dir.mkdir(parents=True, exist_ok=True)
        result: dict[str, str] = {}
        for tier, workflow in templates.items():
            content = yaml.dump(workflow, default_flow_style=False, sort_keys=False)
            out_path = out_dir / f"{tier}.yml"
            out_path.write_text(content)
            result[tier] = content

        log_event("governance.ci_templates_generated", {"tiers": list(templates.keys())})
        return result

    # ----- M10: GitHub Queue Push -----

    def push_queue_to_github(self, max_items: int = 5, dry_run: bool = True) -> list[dict]:
        """Create GitHub issues from top-priority work queue items.

        Returns list of created/would-create issue dicts.
        """
        from .work_item import WorkRegistry

        wr = WorkRegistry()
        open_items = [
            item for item in wr.items.values()
            if item.status == "OPEN"
        ]
        # Sort by score descending
        open_items.sort(key=lambda x: -x.score)
        top_items = open_items[:max_items]

        results: list[dict] = []
        for item in top_items:
            # Determine the target repo for the issue
            org = ""
            for meta in ORGANS.values():
                if meta["registry_key"] == item.organ:
                    org = meta["org"]
                    break
            target_repo = f"{org}/{item.repo}" if org and item.repo else f"{org}/.github" if org else ""

            body = (
                f"## Work Queue Item\n\n"
                f"**ID:** `{item.id}`\n"
                f"**Organ:** {item.organ}\n"
                f"**Repo:** {item.repo or 'N/A'}\n"
                f"**Priority:** {item.priority}\n"
                f"**Score:** {item.score}\n"
                f"**Category:** {item.category}\n\n"
                f"### Description\n\n{item.description}\n\n"
                f"### Suggested Command\n\n```\n{item.suggested_command}\n```\n\n"
                f"*Generated by `conductor queue push`*"
            )
            title = f"[conductor] {item.description[:70]}"
            ...
            if not dry_run and target_repo:
                try:
                    result = subprocess.run(
                        ["gh", "issue", "create",
                         "--repo", target_repo,
                         "--title", title,
                         "--body", body,
                         "--label", "conductor-queue"],
                        capture_output=True, text=True, timeout=15,
                    )
                    if result.returncode == 0:
                        issue_info["created"] = True
                        issue_info["url"] = result.stdout.strip()
                    else:
                        issue_info["error"] = result.stderr.strip()
                except (subprocess.TimeoutExpired, FileNotFoundError) as exc:
                    issue_info["error"] = str(exc)

            results.append(issue_info)

        log_event("governance.queue_push", {
            "dry_run": dry_run,
            "items": len(results),
